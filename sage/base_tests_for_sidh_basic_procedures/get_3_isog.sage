...
def test_single_get_3_isog(arithmetic_parameters, fp2, test_value_x3, test_value_x3i, test_value_z3, test_value_z3i):
    prime = arithmetic_parameters[3]
    
    test_value_x3_mont    = enter_montgomery_domain(arithmetic_parameters, test_value_x3)
    test_value_x3i_mont   = enter_montgomery_domain(arithmetic_parameters, test_value_x3i)
    ...
    test_value_o1_mont, test_value_o1i_mont, test_value_o2_mont, test_value_o2i_mont, test_value_o3_mont, test_value_o3i_mont, test_value_o4_mont, test_value_o4i_mont = get_3_isog(arithmetic_parameters, test_value_x3_mont, test_value_x3i_mont, test_value_z3_mont, test_value_z3i_mont)
    ...
    test_value_final_t2 = [test_value_o3, test_value_o3i]
    test_value_final_t4 = [test_value_o4, test_value_o4i]
    
    true_value_final = sage_get_3_isog(fp2, test_value_x3, test_value_x3i, test_value_z3, test_value_z3i)
    
    true_value_final_t1 = true_value_final[0:2]
    true_value_final_t3 = true_value_final[2:4]
    true_value_final_t2 = true_value_final[4:6]
    true_value_final_t4 = true_value_final[6:8]
    
    if((test_value_final_t1[0] != true_value_final_t1[0]) or (test_value_final_t1[1] != true_value_final_t1[1]) or (test_value_final_t3[0] != true_value_final_t3[0]) or (test_value_final_t3[1] != true_value_final_t3[1]) or (test_value_final_t2[0] != true_value_final_t2[0]) or (test_value_final_t2[1] != true_value_final_t2[1]) or (test_value_final_t4[0] != true_value_final_t4[0]) or (test_value_final_t4[1] != true_value_final_t4[1])):
        print("Error during the get 3 isogenies procedure ")
        print('')
        print("Prime")
        print(prime)
        print('')
...
def test_get_3_isog(base_word_size, extended_word_size, prime_size_bits, number_of_bits_added, accumulator_word_size, prime, number_of_tests):
    ...
    maximum_tests_input = []
    for test_value_x3 in maximum_tests:
        for test_value_x3i in maximum_tests:
            for test_value_z3 in maximum_tests:
                for test_value_z3i in maximum_tests:
                    maximum_tests_input += [[test_value_x3, test_value_x3i, test_value_z3, test_value_z3i]]
    
    for test in maximum_tests_input:
        error_computation = test_single_get_3_isog(arithmetic_parameters, fp2, test[0], test[1], test[2], test[3])
    ...
    if(not error_computation):
        for i in range(number_of_tests):
            if(((i %(1000)) == 0)):
                print(i)
            test_value_x3   = randint(1, max_value-1)
            test_value_x3i  = randint(1, max_value-1)
            test_value_z3   = randint(1, max_value-1)
            test_value_z3i  = randint(1, max_value-1)
            
            error_computation = test_single_get_3_isog(arithmetic_parameters, fp2, test_value_x3, test_value_x3i, test_value_z3, test_value_z3i)
            if(error_computation):
                break
        
    return error_computation

def print_VHDL_get_3_isog_test(VHDL_memory_file_name, base_word_size, extended_word_size, prime_size_bits, number_of_bits_added, accumulator_word_size, prime, number_of_tests):
    ...
    for test in maximum_tests_input:
        ...
        test_value_x3_list = integer_to_list(extended_word_size_signed, number_of_words, test[0])
        test_value_x3i_list = integer_to_list(extended_word_size_signed, number_of_words, test[1])
        test_value_z3_list = integer_to_list(extended_word_size_signed, number_of_words, test[2])
        test_value_z3i_list = integer_to_list(extended_word_size_signed, number_of_words, test[3])
        
        test_value_x3_mont    = enter_montgomery_domain(arithmetic_parameters, test[0])
        test_value_x3i_mont   = enter_montgomery_domain(arithmetic_parameters, test[1])
        test_value_z3_mont    = enter_montgomery_domain(arithmetic_parameters, test[2])
        test_value_z3i_mont   = enter_montgomery_domain(arithmetic_parameters, test[3])
        ...
        print_list_convert_format_VHDL_MAC_memory(VHDL_memory_file, base_word_size_signed, base_word_size_signed_number_words, extended_word_size_signed, test_value_x3_list, maximum_number_of_words)
        print_list_convert_format_VHDL_MAC_memory(VHDL_memory_file, base_word_size_signed, base_word_size_signed_number_words, extended_word_size_signed, test_value_x3i_list, maximum_number_of_words)
    ...
    for i in range(tests_already_performed, number_of_tests):
        ...
        test_value_x3_list  = integer_to_list(extended_word_size_signed, number_of_words, test_value_x3)
        test_value_x3i_list = integer_to_list(extended_word_size_signed, number_of_words, test_value_x3i)
...
def load_VHDL_get_3_isog_test(VHDL_memory_file_name, base_word_size, extended_word_size, prime_size_bits, number_of_bits_added, accumulator_word_size, prime, number_of_tests = 0, debug_mode=False):
    ...
    if(loaded_prime != prime):
        print("Error in get 3 isogenies computation : " + str(current_test))
    ...
    while(current_test != (number_of_tests-1)):
        test_value_x3         = load_list_value_VHDL_MAC_memory_as_integer(VHDL_memory_file, base_word_size_signed, base_word_size_signed_number_words, maximum_number_of_words, False)
        test_value_x3i        = load_list_value_VHDL_MAC_memory_as_integer(VHDL_memory_file, base_word_size_signed, base_word_size_signed_number_words, maximum_number_of_words, False)
...
def test_all_get_3_isog(base_word_size_signed, extended_word_size_signed, number_of_bits_added, accumulator_word_size, primes, number_of_tests):
    error_computation = False
    for i in range(len(primes)):
        prime = primes[i]
        print("Testing prime : " + str(prime))
        prime_size_bits = int(prime).bit_length()
        error_computation = test_get_3_isog(base_word_size_signed, extended_word_size_signed, prime_size_bits, number_of_bits_added, accumulator_word_size, prime, number_of_tests)
        if(error_computation):
            break
        
def print_all_VHDL_get_3_isog_test(base_word_size_signed, extended_word_size_signed, number_of_bits_added, accumulator_word_size, primes, VHDL_file_names, number_of_tests):
    for i in range(len(primes)):
        prime = primes[i]
        VHDL_file_name = VHDL_file_names[i]
        print("Printing prime : " + str(prime))
        prime_size_bits = int(prime).bit_length()
        print_VHDL_get_3_isog_test(VHDL_file_name, base_word_size_signed, extended_word_size_signed, prime_size_bits, number_of_bits_added, accumulator_word_size, prime, number_of_tests)

def load_all_VHDL_get_3_isog_test(base_word_size_signed, extended_word_size_signed, number_of_bits_added, accumulator_word_size, primes, VHDL_file_names):
    for i in range(len(primes)):
        prime = primes[i]
        VHDL_file_name = VHDL_file_names[i]
        print("Loading prime test of : " + str(prime))
        prime_size_bits = int(prime).bit_length()
        load_VHDL_get_3_isog_test(VHDL_file_name, base_word_size_signed, extended_word_size_signed, prime_size_bits, number_of_bits_added, accumulator_word_size, prime)
