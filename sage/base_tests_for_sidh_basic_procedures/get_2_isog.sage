...
def test_single_get_2_isog(arithmetic_parameters, fp2, test_value_x2, test_value_x2i, test_value_z2, test_value_z2i):
    ...
    test_value_o1_mont, test_value_o1i_mont, test_value_o2_mont, test_value_o2i_mont = get_2_isog(arithmetic_parameters, test_value_x2_mont, test_value_x2i_mont, test_value_z2_mont, test_value_z2i_mont)
    ...
    true_value_final = sage_get_2_isog(fp2, test_value_x2, test_value_x2i, test_value_z2, test_value_z2i)
    
    true_value_final_t2 = true_value_final[0:2]
    true_value_final_t1 = true_value_final[2:4]
    
    if((test_value_final_t2[0] != true_value_final_t2[0]) or (test_value_final_t2[1] != true_value_final_t2[1]) or (test_value_final_t1[0] != true_value_final_t1[0]) or (test_value_final_t1[1] != true_value_final_t1[1])):
        print("Error during the get 2 isogenies procedure ")
        print('')
        print("Prime")
        print(prime)
        print('')
...
def test_get_2_isog(base_word_size, extended_word_size, prime_size_bits, number_of_bits_added, accumulator_word_size, prime, number_of_tests):
        ...
        for test_value_x2i in maximum_tests:
            for test_value_z2 in maximum_tests:
                for test_value_z2i in maximum_tests:
                    maximum_tests_input += [[test_value_x2, test_value_x2i, test_value_z2, test_value_z2i]]
    for test in maximum_tests_input:
        error_computation = test_single_get_2_isog(arithmetic_parameters, fp2, test[0], test[1], test[2], test[3])
    ...
    if(not error_computation):
        for i in range(number_of_tests):
            ...
            error_computation = test_single_get_2_isog(arithmetic_parameters, fp2, test_value_x2, test_value_x2i, test_value_z2, test_value_z2i)
            if(error_computation):
                break
        
    return error_computation

def print_VHDL_get_2_isog_test(VHDL_memory_file_name, base_word_size, extended_word_size, prime_size_bits, number_of_bits_added, accumulator_word_size, prime, number_of_tests):
...
def load_VHDL_get_2_isog_test(VHDL_memory_file_name, base_word_size, extended_word_size, prime_size_bits, number_of_bits_added, accumulator_word_size, prime, number_of_tests = 0, debug_mode=False):
    ...
    if(loaded_prime != prime):
        print("Error in get 2 isogenies computation : " + str(current_test))
    ...
    while(current_test != (number_of_tests-1)):
        ...
        computed_test_value_o1_mont, computed_test_value_o1i_mont, computed_test_value_o2_mont, computed_test_value_o2i_mont = get_2_isog(arithmetic_parameters, test_value_x2_mont, test_value_x2i_mont, test_value_z2_mont, test_value_z2i_mont)
...
def test_all_get_2_isog(base_word_size_signed, extended_word_size_signed, number_of_bits_added, accumulator_word_size, primes, number_of_tests):
    error_computation = False
    for i in range(len(primes)):
        prime = primes[i]
        print("Testing prime : " + str(prime))
        prime_size_bits = int(prime).bit_length()
        error_computation = test_get_2_isog(base_word_size_signed, extended_word_size_signed, prime_size_bits, number_of_bits_added, accumulator_word_size, prime, number_of_tests)
        if(error_computation):
            break
        
def print_all_VHDL_get_2_isog_test(base_word_size_signed, extended_word_size_signed, number_of_bits_added, accumulator_word_size, primes, VHDL_file_names, number_of_tests):
    for i in range(len(primes)):
        prime = primes[i]
        VHDL_file_name = VHDL_file_names[i]
        print("Printing prime : " + str(prime))
        prime_size_bits = int(prime).bit_length()
        print_VHDL_get_2_isog_test(VHDL_file_name, base_word_size_signed, extended_word_size_signed, prime_size_bits, number_of_bits_added, accumulator_word_size, prime, number_of_tests)

def load_all_VHDL_get_2_isog_test(base_word_size_signed, extended_word_size_signed, number_of_bits_added, accumulator_word_size, primes, VHDL_file_names):
    for i in range(len(primes)):
        prime = primes[i]
        VHDL_file_name = VHDL_file_names[i]
        print("Loading prime test of : " + str(prime))
        prime_size_bits = int(prime).bit_length()
        load_VHDL_get_2_isog_test(VHDL_file_name, base_word_size_signed, extended_word_size_signed, prime_size_bits, number_of_bits_added, accumulator_word_size, prime)
